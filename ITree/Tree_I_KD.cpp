...
void KD_TREE<PointType>::start_thread(){
    ...
    // start thread
    rebuild_thread = std::thread(multi_thread_ptr, this);
    std::cout<<"Multi thread started"<<std::endl;
}
...
void KD_TREE<PointType>::multi_thread_rebuild() {
    termination_flag_mutex_lock.lock();
    bool terminated = termination_flag;
    termination_flag_mutex_lock.unlock();
    while (!terminated) {
        rebuild_ptr_mutex_lock.lock();
        if (Rebuild_Ptr != nullptr) {
            ...
            (*Rebuild_Ptr)->working_flag = true;
            working_flag_mutex.unlock();
            rebuild_ptr_mutex_lock.lock();
            KD_TREE_NODE *old_root_node = *Rebuild_Ptr;
            KD_TREE_NODE *father_ptr = old_root_node->father_ptr;
            rebuild_ptr_mutex_lock.unlock();
            /* Rebuild the tree */
            working_flag_mutex.lock();
            // Lock Search
            ...
            if (int(Rebuild_PCL_Storage.size()) > 0) {
                ...
                while (!Rebuild_Logger.empty()) {
                    ...
                    tmp_counter++;
                    if (tmp_counter % 10 == 0) std::this_thread::sleep_for(std::chrono::seconds(1));
                    working_flag_mutex.lock();
                    rebuild_logger_mutex_lock.lock();
                }
                rebuild_logger_mutex_lock.unlock();
...
void KD_TREE<PointType>::Add_Point_Boxes(vector<BoxPointType> & BoxPoints){
    for (auto & BoxPoint : BoxPoints){
        if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node){
            Add_by_range(&Root_Node ,BoxPoint, true);
        } else {
            Operation_Logger_Type operation;
            operation.boxpoint = BoxPoint;
            operation.op = ADD_BOX;
            working_flag_mutex.lock();
            Add_by_range(&Root_Node ,BoxPoint, false);
            if (rebuild_flag){
                working_flag_mutex.lock();
                Rebuild_Logger.push(operation);
                working_flag_mutex.unlock();
...
int KD_TREE<PointType>::Delete_Point_Boxes(vector<BoxPointType> & BoxPoints){
    ...
    for (auto & BoxPoint : BoxPoints){
        if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node){
            tmp_counter += Delete_by_range(&Root_Node ,BoxPoint, true, false);
        } else {
            Operation_Logger_Type operation;
            operation.boxpoint = BoxPoint;
            operation.op = DELETE_BOX;
            working_flag_mutex.lock();
            tmp_counter += Delete_by_range(&Root_Node ,BoxPoint, false, false);
            if (rebuild_flag){
                working_flag_mutex.lock();
                Rebuild_Logger.push(operation);
                working_flag_mutex.unlock();
...
void KD_TREE<PointType>::Rebuild(KD_TREE_NODE ** root){
    ...
    if ((*root)->left_son_ptr != nullptr) Rebuild(&((*root)->left_son_ptr));
    if ((*root)->right_son_ptr != nullptr) Rebuild(&((*root)->right_son_ptr));
    father_ptr = (*root)->father_ptr;
    if (father_ptr != nullptr){
        if (father_ptr->left_son_ptr == (*root)){
            father_ptr->left_son_ptr = nullptr;
        } else {
            father_ptr->right_son_ptr = nullptr;
        }
    }
    delete (*root);
    *root = nullptr;
...
void KD_TREE<PointType>::Add_by_range(KD_TREE_NODE ** root, BoxPointType boxpoint, bool allow_rebuild){
    ...
    if (boxpoint.vertex_min[0] <= (*root)->node_range_x[0] && boxpoint.vertex_max[0] > (*root)->node_range_x[1] && boxpoint.vertex_min[1] <= (*root)->node_range_y[0] && boxpoint.vertex_max[1]> (*root)->node_range_y[1] && boxpoint.vertex_min[2] <= (*root)->node_range_z[0] && boxpoint.vertex_max[2] > (*root)->node_range_z[1]){
        (*root)->tree_deleted = (*root)->tree_downsample_deleted;
