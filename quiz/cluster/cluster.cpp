...
pcl::visualization::PCLVisualizer::Ptr initScene(Box window, int zoom)
{
    ...
    viewer->setCameraPosition(0, 0, zoom, 0, 0, 0);
    viewer->addCoordinateSystem(1.0);

    viewer->addCube(window.x_min, window.x_max, window.y_min, window.y_max, window.z_min, window.z_max);
...
void render2DTree(Node* node, pcl::visualization::PCLVisualizer::Ptr& viewer, Box window, int& iteration, int depth=0)
{
    if (node != NULL)
    {
        ...
        if (depth % 2 == 0)
        {
            ...
            upperwindow.x_max = node->point[0];
            lowerWindow.x_min = node->point[0];
        }
        ...
        else
        {
            ...
            upperwindow.y_max = node->point[1];
            lowerWindow.y_min = node->point[1];
        }
        iteration++;
...
int main()
{
    ...
    std::vector<int> nearby = tree->search({-3, -3}, 3.0);
    ...
    for (std::vector<int> cluster: clusters)
    {
        ...
        renderPointCloud(viewer, clusterCloud, "cluster"+std::to_string(clusterId), colors[clusterId]);
        ++clusterId;
